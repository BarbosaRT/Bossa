  ...
  group('JustPlaylistManager', () {
    ...
    test('setPlaylist', () async {
      ...
      Future.delayed(const Duration(seconds: 2)).then(
        (value) {
          expect(manager.player.sequence.length, 2);
          expect(manager.player.sequence[0],
              AudioSource.uri(Uri.parse(playlist.songs[0].url)));

          expect(manager.player.sequence[1],
    ...
    test('seekToNext', () async {
      ...
      expect(
          pmanager.player.sequence[pmanager.player.currentIndex!],
          AudioSource.uri(Uri.parse(playlist.songs[1].url)));
    ...
    test('seekToPrevious', () async {
      ...
      expect(manager.player.sequence[manager.player.currentIndex!],
          AudioSource.uri(Uri.parse(playlist.songs[0].url)));
    });
